...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    $sql = "SELECT * FROM RSO_Events WHERE EventId='$EventId' OR Rso_Id='$Rso_Id'";
    $result = mysqli_query($conn, $sql);
    if (mysqli_num_rows($result) > 0) {
        $error = "Event already exists.";
    } else if(mysqli_num_rows($result) < 0){
        // Insert data into database
        $sql = "INSERT INTO RSO_Events (Uid, Rso_Id, EventId, Name, Location, Date, Time, Category) VALUES ('$Uid', '$Rso_Id', '$EventId', '$Name', '$Location', '$Date', '$Time', '$Category')";
        $result = mysqli_query($conn, $sql);
        if ($result) {
